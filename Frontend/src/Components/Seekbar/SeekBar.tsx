...
const SeekBar = ({seeker}:{seeker:Function}) => {
  ...
  return (
    <div className='flex text-sm gap-[1rem] justify-center items-center mt-[3rem] relative'>
            <p className='absolute left-[-15%]'>{durationCalculator(Math.floor(duration))}</p>
            <div className='w-[22rem] seek h-[0.2rem] bg-[#DADADA] cursor-pointer' onClick={(e)=>seeker(e)} onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}>
